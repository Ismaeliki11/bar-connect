...
export default function DineroPage() {
  ...
  // Pre-fill realistic mock shift data for the last 3 days if empty
  const prefillMockData = () => {
    ...
    const mockEntries = [
      {
        ...
        cash: 340,
        card: 480,
        notes: "Turno de mañana concurrido. Desayunos y reposición de almacén completa.",
        by: "Clemen" as const,
      },
      {
        offsetDays: 1,
        ...
        cash: 490,
        card: 690,
        notes: "Excelente tarde-noche. Muchas tapas y buen ambiente de fútbol.",
        by: "Isabel" as const,
      },
      {
        offsetDays: 2,
        ...
        cash: 290,
        card: 410,
        notes: "Mañana estándar. Limpieza a fondo de cafetera y vaporizadores.",
        by: "Isabel" as const,
      },
      {
        offsetDays: 2,
        ...
        cash: 380,
        card: 520,
        notes: "Todo en orden. Fin de turno limpio.",
        by: "Clemen" as const,
      },
      {
        offsetDays: 3,
        ...
        cash: 220,
        card: 320,
        notes: "Lluvia a primera hora, menos desayunos de lo habitual.",
        by: "Clemen" as const,
      },
      {
        offsetDays: 3,
        ...
        cash: 310,
        card: 410,
        notes: "Cierre tranquilo. Mesas de terraza recogidas a su hora.",
        by: "Isabel" as const,
      },
    ];

    let dataChanged = false;
    mockEntries.forEach(({ offsetDays, shift, cash, card, notes, by }) => {
      const d = new Date();
      d.setDate(today.getDate() - offsetDays);
      const mDateStr = getDateString(d);
      const key = `caja_closure_${mDateStr}_${shift}`;
      ...
      if (!localStorage.getItem(key)) {
        const closure: ShiftClosure = {
          ...
          submittedBy: by,
          submittedAt: new Date(d.setHours(shift === "manana" ? 16 : 23, 15, 0, 0)).toISOString(),
          notes,
        };
        localStorage.setItem(key, JSON.stringify(closure));
        dataChanged = true;
      }
    });
  ...
  const loadClosuresData = () => {
    ...
    // Check if we need to prefill mock data
    prefillMockData();
